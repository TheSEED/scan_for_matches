...
int parse_cmd_ns PROTO((char *line, punit_t pu_s[], char cv[], int iv[]));
int build_conversion_tables PROTO((void));
char *wt_pat PROTO((int *maxwt,int *cutoff,int *tupsz,int **t,int *n,char *p));
...
int compl_flag,rule_set;
{
	...
	return two_data-start_two_data;
    }

    nxtent=0;
    while (two_len || nxtent)
    {
        if (two_len && KnownChar((*two_data)&15) &&
