...
int main(int argc,char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "pcnm:i:")) != -1)
	switch (c) {
	  case 'c':
	    complements=1;
	    break;
	...
	fprintf(stderr, "usage: scan_for_matches -c [for complementary strand] -p [protein][-n stop_after_n_misses] [-m max_hits] [-i file_of_ids_to_ignore] pattern_file < fasta_input > hits\n");
	exit (2);
    }
...
/*		 i = next_match(hits))     to get overlapping hits */
		...
		printf(">%s:[%zd,%zd]\n",id,(size_t) (1+hits[0]-cdata), (size_t) (1+(hits[i] - 1 - cdata)));
		for (i1=0; i1 < i; i1++)
		{
		    j = hits[i1+1] - hits[i1];
		    ...
		    /*		 i = next_match(hits))     to get overlapping hits */
		{
		    ...
		    printf(">%s:[%zd,%zd]\n",id, (size_t) 1+(ln-1) - (hits[0]-cdata), (size_t) 1+(ln-1) - (hits[i] - 1 - cdata));
		    for (i1=0; i1 < i; i1++)
		    {
			j = hits[i1+1] - hits[i1];
			for (p=data + (hits[i1]-cdata); j; j--)
